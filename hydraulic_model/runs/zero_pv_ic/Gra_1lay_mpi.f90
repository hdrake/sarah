...
!***************************************************************************
 ...
 real(fpp), parameter :: eps=0.0000002_fpp,eps8=0.00000001_fpp
 ...
 real(fpp), parameter :: five=5._fpp, six=6._fpp, seven=7._fpp, eight=8._fpp
...
!          Apply boundary conditions, including weird form of radiation condition (?)
...
!          Don't apply radiation BC again yet (why?)
...
!      # Set initial conditions for q.
       ...
       real(fpp) :: aa, ee, BBB, QQQ, del0, lambda

       real(fpp) :: bb(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    pp(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    qq(sx-mbc:ex+mbc,sy-mbc:ey+mbc)

       real(fpp) :: del1(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    TT(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    SS(sx-mbc:ex+mbc,sy-mbc:ey+mbc)

       real(fpp) :: x3(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    x4(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    w3(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    w4(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
                    ww(sx-mbc:ex+mbc,sy-mbc:ey+mbc), &
...
!           # calculate critical parameters
            ...
            pp(i,j) = (-three*bb(i,j)**2)/(eight*aa**2)
            qq(i,j) = (bb(i,j)**3)/(eight*aa**3)
            
            del1(i,j) = 27.0_fpp*(bb(i,j)**2)*ee

            TT(i,j) = ((del1(i,j)+(del1(i,j)**2 &
                        - four*del0**3)**(half))/two)**(one/three)
            SS(i,j) = (half)*(-(two/three)*pp(i,j) &
                        + (one/(three*aa))*(TT(i,j) + del0/TT(i,j)))**(half)

            x3 = -bb(i,j)/(four*aa) + SS(i,j) + half*(-four*SS(i,j)**2 &
                   - 2*pp(i,j) - qq(i,j)/SS(i,j))**half
            x4 = -bb(i,j)/(four*aa) + SS(i,j) - half*(-four*SS(i,j)**2 &
                   - 2*pp(i,j) - qq(i,j)/SS(i,j))**half
