 ...
 contains
   ...
   subroutine m_parameters_init  !============================
    ...
    open(10,file=fname,status='unknown',form='formatted') 
      ...
      read(10,*)  ixbc, iybc
      read(10,*)  gamma, cf, cb
      read(10,*)  alpha, xc, wc, ramp1       ! pchan stab    
      read(10,'(a)') outfile
      read(10,'(a)') restart
    close(10)
...
!     # Set initial conditions for q.
       ...
       yc = -xc ! conversion between Larry's and my coordinate systems
       ...
       do j=sy-mbc,ey+mbc
         do i=sx-mbc,ex+mbc
!           # set topography
            b(i,j) = 0.2*(dexp(-(0.125*(x(i)-60.))**2) -1) +alpha*y(j)**2
            cbxy(i,j) = cb
            yp = y(j) - yc
            if (dabs(yp) .lt. half*wc) then
              CALL RANDOM_NUMBER(HARVEST = rn)	       
	          hp = ramp1*dsqrt(-two*dlog(rn(1)))*dcos(twopi*rn(2))
              q(i,j,1) = (half+alpha)*(quarter*wc**2-yp**2) + hp
              q(i,j,2) = q(i,j,1)*(yp - two*alpha*yc)
            endif              
          enddo
        enddo
